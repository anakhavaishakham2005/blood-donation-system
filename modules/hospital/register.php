...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt->execute();
    $stmt->store_result();
    if ($stmt->num_rows > 0) $errors[] = "Email already registered for a hospital.";
    $stmt->close();

    if (empty($errors)) {
        $hash = password_hash($password, PASSWORD_DEFAULT);
        $stmt = $conn->prepare("INSERT INTO hospitals (name, email, password, phone, address, city) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param('ssssss', $name, $email, $hash, $phone, $address, $city);
        if ($stmt->execute()) {
            $success = "Hospital registered successfully. You can login now.";
        } else {
...
<div class="card">
  <div class="card-body">
    ...
    <form method="post" class="row g-3">
      ...
      <div class="col-12">
        <button class="btn btn-danger">Register Hospital</button>
        <a href="/blood-donation-system/modules/hospital/login.php" class="btn btn-link">Already have account? Login</a>
      </div>
    </form>
  </div>
</div>
